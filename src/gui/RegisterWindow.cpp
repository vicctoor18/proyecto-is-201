...
RegisterWindow::RegisterWindow(QWidget *parent) : QWidget(parent) {
  ...
  connect(registerButton, &QPushButton::clicked, this, &RegisterWindow::onRegisterClicked); // Conecta el botón con la función onRegisterClicked
}
...
void RegisterWindow::onRegisterClicked() { // Función que pasa la información escrita a un usuario
  ...
  if (username.isEmpty() || password.isEmpty() || name.isEmpty() || degree.isEmpty() || nationality.isEmpty()) {
    statusLabel->setText("Todos los campos son obligatorios");
    return;
  }

  if (DatabaseManager::instance().registerUser(username, password, role, name, degree, nationality)) {
    QMessageBox::information(this, "Exito","Usuario registrado correctamente. Ahora puede iniciar sesion."); // Si se ha creado con éxito
    this->close();
  } else {
